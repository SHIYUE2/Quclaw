...
function getVisionConfig() {
  const config = getConfig();
  // Priority: uclaw.vision > tools.media.image > env
  const vision = config.uclaw?.vision;
  if (vision?.apiKey) {
    return {
      apiKey: vision.apiKey,
      baseUrl: vision.baseUrl || 'https://dashscope.aliyuncs.com/compatible-mode/v1',
      model: vision.model || 'qwen-vl-max',
    };
  }
  // Fallback: check tools.media.image.models
  const mediaModels = config.tools?.media?.image?.models;
  if (Array.isArray(mediaModels) && mediaModels.length > 0) {
    const m = mediaModels[0];
    return {
      apiKey: m.apiKey || config.env?.OPENAI_API_KEY || config.env?.DASHSCOPE_API_KEY || '',
      baseUrl: m.baseUrl || 'https://dashscope.aliyuncs.com/compatible-mode/v1',
      model: m.model || 'qwen-vl-max',
    };
  }
  return { apiKey: '', baseUrl: '', model: 'qwen-vl-max' };
}

// Load image-inject.js content for injection into webview
let _imageInjectCode = null;
function getImageInjectCode() {
  if (_imageInjectCode) return _imageInjectCode;
  try {
    _imageInjectCode = fs.readFileSync(
      path.join(__dirname, 'image-inject.js'),
      'utf8',
    );
  } catch {
    _imageInjectCode = '';
    console.warn(`[${APP_NAME}] image-inject.js not found`);
  }
  return _imageInjectCode;
}

// Load branding-inject.js content for injection into webview
let _brandingInjectCode = null;
function getBrandingInjectCode() {
  if (_brandingInjectCode) return _brandingInjectCode;
  try {
    _brandingInjectCode = fs.readFileSync(
      path.join(__dirname, 'branding-inject.js'),
      'utf8',
    );
  } catch {
    _brandingInjectCode = '';
    console.warn(`[${APP_NAME}] branding-inject.js not found`);
  }
  return _brandingInjectCode;
}
...
function createWindow() {
  ...
  // Inject image upload support into the gateway dashboard
  mainWindow.webContents.on('did-finish-load', () => {
    const url = mainWindow.webContents.getURL();
    // Only inject into the gateway dashboard (port-based URL), not config or loading pages
    if (url.includes(`127.0.0.1:${gatewayPort}`) || url.includes(`localhost:${gatewayPort}`)) {
      const brandingCode = getBrandingInjectCode();
      if (brandingCode) {
        mainWindow.webContents.executeJavaScript(brandingCode).catch((err) => {
          console.error(`[${APP_NAME}] branding-inject injection failed:`, err.message);
        });
      }
      const imgCode = getImageInjectCode();
      if (imgCode) {
        mainWindow.webContents.executeJavaScript(imgCode).catch((err) => {
          console.error(`[${APP_NAME}] image-inject injection failed:`, err.message);
        });
      }
    }
  });
  ...
  const template = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'Website',
          click: () => shell.openExternal('https://qu_claw.org')
        },
        {
...
function setupIPC() {
  ...
  // ── Image Analysis IPC ──
  ipcMain.handle('analyze-image', async (_evt, base64, mimeType) => {
    const vision = getVisionConfig();
    return analyzeImage(base64, mimeType || 'image/png', vision);
  });

  ipcMain.handle('get-vision-config', () => {
    const v = getVisionConfig();
    return { hasKey: !!v.apiKey, model: v.model };
  });
